...
freqFunNonPara <- function(data, n.boot, estimates, interval, omega.freq.method,
  ...
  if (("alpha" %in% estimates & !alpha.int.analytic) |
    ...
    for (i in 1:n.boot) {
      ...
      boot_cov[i, , ] <- cov(boot_data, use = use_cases)
      callback()
    }
  ...
  if ("glb" %in% estimates) {
    ...
    if (length(unique(round(glb_obj, 4))) == 1){
      ...
      res$conf$low$freq_glb <- quantile(glb_obj, probs = probs[1], na.rm = T)
      res$conf$up$freq_glb <- quantile(glb_obj, probs = probs[2], na.rm = T)
    }
